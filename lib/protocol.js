...
function Protocol(store, connection)
{
...
{
  debug(packet)
  let connect = handler.Connect;
  connect.onConnect(packet, this._conn)
}
...
{
  debug(packet)
  handler.Ping(this._conn, this._session)
}
...
{
  debug(packet)
  handler.Disconnect(this._conn, this._session, this._sessionStore)
